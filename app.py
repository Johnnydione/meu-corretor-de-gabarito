...
if foto_upload and nome_aluno:
    ...
    # 4. DEFINIR ÁREA DE LEITURA (ROI) COM AJUSTE DE VÉRTICE
    if len(quadrados) >= 4:
        all_pts = np.concatenate(quadrados)
        x, y, w_box, h_box = cv2.boundingRect(all_pts)
        
        # --- AJUSTE DE PRECISÃO PARA PARTIR DO VÉRTICE ---
        # Aumentei os offsets para que o azul comece DEPOIS do quadrado preto
        offset_y = int(h_box * 0.065) # Aumentado de 0.052 para 0.065
        offset_x = int(w_box * 0.045) # Aumentado de 0.035 para 0.045
        ...
        # Desenha o retângulo azul (Agora deve estar dentro dos quadrados)
        cv2.rectangle(img_viz, (x_final, y_final), (x_final+w_final, y_final+h_final), (255, 0, 0), 3)
    else:
        st.warning("⚠️ Quadrados não detectados!")
        roi_thresh = thresh
        img_viz_roi = img_viz
